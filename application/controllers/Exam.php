...
class Exam extends Admin_Controller
{
    ...
    public function index()
    {
     ...
     public function onlineexamlist()
    {
        if (!get_permission('exam', 'is_view')) {
            access_denied();
        }       
        ...
        $this->data['branch_id'] = $this->application_model->get_branch_id();
        $this->data['examlist'] = $this->exam_model->getonlineExamList();
        $this->data['title'] = translate('Online Exam');
        $this->data['sub_page'] = 'exam/onlineexamlist';
        $this->data['main_menu'] = 'exam';
        $this->load->view('layout/index', $this->data);
    }
	...
	public function quiztest1($id) {
	
	
	
	 $current_time =  date("Y-m-d H:i:s");
	 $sccode = $this->application_model->get_branch_id();
		   $this->session->set_userdata("start_time_quiz", $current_time);
		    $listime = $this->exam_model->getexamtime($id);
		     $data= array(
                'scode' => $sccode,
                'studid' => get_loggedin_user_id(),
                'queid' => '',
                'category_id' => $listime['id'],
                'classname' => $listime['classid'],
                'subjectname' => $listime['subject'],
                'answer' => '',
                'score' => ''
            );
       //echo "<pre>"; print_r($data);        
	   //$this->exam_model->quizaddrecord($data);
		   
      redirect('exam/quiztest/'.$id);	
     ...
     public function quiztest($id) {
        ...
        $data['title_list'] = 'Quiz List';
        $list = $this->exam_model->getQuizlist($id);
		//print_r($list); die;
		$data['categoryid'] = $id;
        $data['list'] = $list;
         $listime = $this->exam_model->getexamtime($id);
         $data['timerdb'] = $listime['timer'];
		 $data['instruction'] = $listime['instruction'];
		 
		 $set_time = $this->session->userdata('start_time_quiz');		  
		 $start = strtotime(date("Y-m-d H:i:s"));
         $end = strtotime($set_time);

       
           $mins = (int)(($start - $end) / 60);
		  $data['timer'] = $listime['timer'] - $mins;
		   $data['sub_page'] = 'exam/quiztestact';
		 
		 
		 
        //$this->load->view('layout/header');
        $this->load->view('exam/quiztest', $data);
        //$this->load->view('layout/footer');
    }
	
	 public function quiztestact() {
     
       $answer = $this->input->post('answer');
       $categoryid = $this->input->post('categoryid');
         $listime = $this->exam_model->getexamtime($categoryid);
      
	 // echo "<pre>"; print_r($this->input->post('answer')); die;
	   $sccode = $this->application_model->get_branch_id();
	  foreach($answer as $key=>$quelist)
	  {
	      $c = 0;
	      $lisright = $this->exam_model->getexamrightans($key); 
	       
	      if(strtolower($lisright['finalans']) == strtolower($quelist[0])) { $c = 1;}
		  if(strtolower($lisright['finalans']) == strtolower($quelist[0])) { $c = 1;}
		  if(strtolower($lisright['finalans']) == strtolower($quelist[0])) { $c = 1;} 
		  if(strtolower($lisright['finalans']) == strtolower($quelist[0])) { $c = 1;}
