...
class Ajax extends MY_Controller
{
   ...
   public function getPrabhagByState()
    {
        $html = "";
        $state_id = $this->application_model->get_branch_id();
        if (!empty($state_id)) {
            $classes = $this->db->select('id,p_name')->where('state_name', $state_id)->get('ekal_prabhag')->result_array();
            if (count($classes)) {
                $html .= "<option value=''>" . translate('select') . "</option>";
                foreach ($classes as $row) {
                    $html .= '<option value="' . $row['id'] . '">' . $row['p_name'] . '</option>';
                }
            } else {
                $html .= '<option value="">' . translate('no_information_available') . '</option>';
            }
        } else {
            $html .= '<option value="">' . translate('select_state_first') . '</option>';
        }
        echo $html;
    }
	
	
	public function getSambhagByPrabhag()
    {
        $html = "";
        $parbhag_id = $this->application_model->get_branch_id();
        if (!empty($parbhag_id)) {
            $classes = $this->db->select('id,s_name')->where('p_id', $parbhag_id)->get('ekal_sambhag')->result_array();
            if (count($classes)) {
                $html .= "<option value=''>" . translate('select') . "</option>";
                foreach ($classes as $row) {
...
public function getBhagBySambhag()
    {
        $html = "";
        $sambhag_id = $this->application_model->get_branch_id();
        if (!empty($sambhag_id)) {
            $classes = $this->db->select('id,b_name')->where('s_id', $sambhag_id)->get('ekal_bhag')->result_array();
            if (count($classes)) {
                $html .= "<option value=''>" . translate('select') . "</option>";
                foreach ($classes as $row) {
                    $html .= '<option value="' . $row['id'] . '">' . $row['b_name'] . '</option>';
                }
            } else {
                $html .= '<option value="">' . translate('no_information_available') . '</option>';
            }
        } else {
            $html .= '<option value="">' . translate('select_sambhag_first') . '</option>';
        }
        echo $html;
    }
...
public function getAnchalByBhag()
    {
        $html = "";
        $bhag_id = $this->application_model->get_branch_id();
        if (!empty($bhag_id)) {
            $classes = $this->db->select('id,a_name')->where('b_id', $bhag_id)->get('ekal_anchal')->result_array();
            if (count($classes)) {
                $html .= "<option value=''>" . translate('select') . "</option>";
                foreach ($classes as $row) {
                    $html .= '<option value="' . $row['id'] . '">' . $row['a_name'] . '</option>';
                }
            } else {
                $html .= '<option value="">' . translate('no_information_available') . '</option>';
            }
        } else {
            $html .= '<option value="">' . translate('select_bhag_first') . '</option>';
        }
        echo $html;
    }

    public function getSanchByAnchal() 
    {
        $html = "";
        $anchal_id = $this->application_model->get_branch_id();
        if (!empty($anchal_id)) {
            $classes = $this->db->select('id,name')->where('a_id', $anchal_id)->get('branch')->result_array();
            if (count($classes)) {
                $html .= "<option value=''>" . translate('select') . "</option>";
                foreach ($classes as $row) {
                    $html .= '<option value="' . $row['id'] . '">' . $row['name'] . '</option>';
            ...
            $html .= '<option value="">' . translate('select_anchal_first') . '</option>';
        }
        echo $html;
    }
	...
	public function prabhag_details()
    {
        if (get_permission('department', 'is_edit')) {
            $id = $this->input->post('id');
            $this->db->where('id', $id);
            $query = $this->db->get('ekal_prabhag');
            $result = $query->row_array();
            echo json_encode($result);
        }
    }

    public function sambhag_details()
    {
        if (get_permission('department', 'is_edit')) {
            $id = $this->input->post('id');
            $this->db->where('id', $id);
            $query = $this->db->get('ekal_sambhag');
            $result = $query->row_array();
            echo json_encode($result);
        }
    }
	
	 public function bhag_details()
    {
        if (get_permission('department', 'is_edit')) {
            $id = $this->input->post('id');
            $this->db->where('id', $id);
            $query = $this->db->get('ekal_bhag');
            $result = $query->row_array();
            echo json_encode($result);
        }
    }
