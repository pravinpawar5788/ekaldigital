...
class Exam_model extends CI_Model
{
    ...
    public function getStudentReportCard($studentID, $examID, $sessionID)
    {
	 ...
	 public function getQuizlist($category) {
		 $db2 = $this->load->database('serverdb', TRUE); 
        $db2->select()->from('quiz_question');
       $db2->where('category_id', $category);
	    $db2->where('writtenexam', 0);
       // $db2->order_by('category_id.id');
	   $query2 = $db2->get();
	  // print_r($db2->last_query()); die;
	   // $sql = "SELECT quiz_question.* FROM `quiz_question` inner join quiz_assign on quiz_question.id = quiz_assign.questionid  WHERE quiz_assign.examid = ". $this->db->escape($category); 
	 ...
	 public function getexamtime($id = null) {
         $db2 = $this->load->database('serverdb', TRUE); 
        $db2->select()->from('quiz_category');
        $db2->where('id', $id);
	 ...
	 public function getexamrightans($id = null) {
         $db2 = $this->load->database('serverdb', TRUE); 
        $db2->select()->from('quiz_question');
        $db2->where('id', $id);
      ...
      public function quizaddrecord($data) {
          $db2 = $this->load->database('serverdb', TRUE);
        $db2->insert('quiz_result', $data);
