...
class Dashboard_model extends CI_Model
{
	...
	public function getvideotraing()
    {
        $this->db->select('*');
        $this->db->from('video_training');
        $this->db->where('video_training.active', 1);
    ...
    public function get_joinuser()
    {
        $sql = "SELECT sum(cast(noofuser as int)) as noofuser FROM `staff` ";  
		
               $query = $this->db->query($sql);

       return  $result2 =  $query->row_array(); 
    }

     public function get_total_empteacher()
    {
        $this->db->select('id');
        $this->db->where("role = '3'");        
        return $this->db->get('login_credential')->num_rows();
    }

    public function get_total_empkarya()
    {
        $this->db->select('id');  
        $this->db->where("role = '9'"); 		
        return $this->db->get('login_credential')->num_rows();
    }
    public function get_total_prabhag()
    {
        $this->db->select('id');        
        return $this->db->get('ekal_prabhag')->num_rows();
    }
	...
	public function get_total_sambhag()
    {
        $this->db->select('id');        
        return $this->db->get('ekal_sambhag')->num_rows();
    }

    public function get_total_bhag()
    {
        $this->db->select('id');        
        return $this->db->get('ekal_bhag')->num_rows();
    }
    
     public function get_total_anchal()
    {
        $this->db->select('id');        
        return $this->db->get('ekal_anchal')->num_rows();
    }
 
    public function get_total_sanch()
    {
        $this->db->select('id');        
        return $this->db->get('branch')->num_rows();
    }
  ...
  $this->db->where("category = '4'"); 		
	 ...
	 public function get_total_samiti()
    {
        $this->db->select('id');   
