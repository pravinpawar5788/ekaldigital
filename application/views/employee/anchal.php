	...
	<div class="col-md-5">
		<section class="panel">
			<header class="panel-heading">
				<h4 class="panel-title"><i class="far fa-edit"></i> <?php echo translate('add') . " " . translate('Anchal(district)'); ?></h4>
			</header>
			<?php echo form_open($this->uri->uri_string()); ?>
				...
				<?php if (is_superadmin_loggedin()): ?>
					...
					<div class="form-group">
						...
						//$arrayPrabhag = $this->app_lib->getClass($branch_id);
						echo form_dropdown("b_name", $arrayPrabhag, set_value('b_name'), "class='form-control' id='b_name'
						data-plugin-selectTwo data-width='100%' data-minimum-results-for-search='Infinity'");
					?>
					</div> 
				...
				<?php endif; ?>
					<div class="form-group mb-md">
						<label class="control-label"><?=translate('Anchal name')?><span class="required">*</span></label>
						<input type="text" class="form-control" name="anchal_name" value="<?php echo set_value('a_name'); ?>" />
						<span class="error"><?=form_error('a_name')?></span>
					</div>
				</div>
						...
						?>
							<tr>
								...
								<?php  endif; if (get_permission('department', 'is_delete')): ?>
									<?php echo btn_delete('employee/anchal_delete/' . $row['id']); ?>
								<?php endif; ?>
								</td>
							</tr>
						<?php
