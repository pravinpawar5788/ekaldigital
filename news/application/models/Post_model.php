...
class Post_model extends CI_Model
{
    ...
    //get popular posts
    public function get_popular_posts($day_count, $lang_id)
    {
        $sql = "SELECT posts.id, posts.title, posts.title_slug, posts.post_type, posts.image_small, posts.image_mime, posts.image_url, users.slug AS author_slug, users.username AS author_username, posts.created_at, hit_counts.count AS pageviews_count,
                (SELECT COUNT(comments.id) FROM comments WHERE posts.id = comments.post_id AND comments.parent_id = 0 AND comments.status = 1) AS comment_count
			    FROM posts 
                INNER JOIN (SELECT COUNT(post_pageviews.post_id) AS count, post_pageviews.post_id FROM post_pageviews WHERE post_pageviews.created_at > DATE_SUB(NOW(), INTERVAL ? DAY) GROUP BY post_pageviews.post_id) AS hit_counts ON hit_counts.post_id = posts.id
                INNER JOIN users ON users.id = posts.user_id
                INNER JOIN categories ON categories.id = posts.category_id
                WHERE posts.is_scheduled = 0 AND posts.visibility = 1 AND posts.status = 1 AND posts.lang_id = ? ORDER BY hit_counts.count DESC LIMIT 5";
        $query = $this->db->query($sql, array(clean_number($day_count), clean_number($lang_id)));
    ...
    public function increase_post_pageviews_count($post)
    {
        if (!empty($post)) {
            if (!isset($_COOKIE['var_post_' . $post->id])) {
                ...
                if ($this->db->insert('post_pageviews', $data)) {
                    helper_setcookie('var_post_' . $post->id, '1');

                    //update post pageviews
                    $sql = "SELECT COUNT(id) AS count FROM post_pageviews WHERE post_id = ?";
                    $query = $this->db->query($sql, array(clean_number($post->id)));
                    $count = $query->row()->count;
                    $data = array(
                        'pageviews' => $count,
                    );
                    $this->db->where('id', clean_number($post->id));
